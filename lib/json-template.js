...
function _Pluralize(value, unused_context, args) {
  ...
  switch (args.length) {
    ...
    default:
      ...
      throw new TypeError('pluralize got too many args');
  }
  return (value > 1) ? p : s;
}
...
function _DoSubstitute(statement, context, callback) {
  ...
  if (value === undefined) {
    throw new ReferenceError(statement.name + ' is not defined');
  }
...
function _Compile(template_str, options) {
  ...
  function GetFormatter(format_str) {
    ...
    if (!pair[0]) {
      throw new Error(format_str + ' is not a valid formatter');
    }
    return pair;
  }
  ...
  function GetPredicate(pred_str) {
    ...
    if (!pair[0]) {
      throw new Error(pred_str + ' is not a valid predicate');
    }
    return pair;
  }
  ...
  if (format_char != ':' && format_char != '|') {
    throw new Error('Only format characters : and | are accepted');
  }

  var meta = options.meta || '{}';
  var n = meta.length;
  if (n % 2 == 1) {
    throw new Error(meta + ' has an odd number of metacharacters');
  }
  ...
  while (true) {
    ...
    if (token.charAt(0) == '.') {  // Keyword
      ...
      if (token == 'end') {
        ...
        if (stack.length > 0) {
          ...
          throw new SyntaxError('Got too many {end} statements');
        }
        continue;
      }
    }
    ...
    if (parts.length == 1) {
      if (default_formatter === null) {
          throw new Error('This template requires explicit formatters.');
      }
  ...
  if (stack.length !== 1) {
    throw new SyntaxError('Got too few {end} statements');
  }
